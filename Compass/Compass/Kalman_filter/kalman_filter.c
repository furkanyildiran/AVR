...
int16_t kalman_calculate(struct KALMAN *kalman, int16_t axis_val, int16_t R, int16_t Q){
	double current_X;
	double current_P;
	cli();
	kalman->K = calc_kalman_k(kalman->previous_P, R);                    //(kalman->previous_P)/(kalman->previous_P + R);
	current_X = calc_current_x(kalman->previous_X, kalman->K, axis_val);//kalman->previous_X + kalman->K*(*axis_val - kalman->previous_X);
